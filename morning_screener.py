...
def parse_backtest_health(path):
    """Parse memory/backtest_results.md for STARK/MITTEL/SCHWACH classification."""
    if not os.path.exists(path):
        return None
    try:
        with open(path) as f:
            content = f.read()
        result = {'STARK': [], 'MITTEL': [], 'SCHWACH': []}
        for line in content.splitlines():
            if not line.startswith('|'):
                continue
            if 'STARK' in line or 'MITTEL' in line or 'SCHWACH' in line:
                # Extract component name (between emoji and parenthesis)
                match = re.search(r'[⚪🟡🟢]\s*(\w+)\s*\(', line)
                if not match:
                    continue
                comp = match.group(1)
                if 'STARK' in line:
                    result['STARK'].append(comp)
                elif 'MITTEL' in line:
                    result['MITTEL'].append(comp)
                elif 'SCHWACH' in line:
                    result['SCHWACH'].append(comp)
...
def build_message(all_data, positions, sector_map, scan_time, total_scanned, pos_dirs, name_map=None, patterns_db=None):
    ...
    # Scoring Health from backtest results
    backtest_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'memory', 'backtest_results.md')
    health = parse_backtest_health(backtest_path)
    if health:
        parts = []
        if health['STARK']:
            parts.append(f'STARK: {", ".join(health["STARK"])}')
        if health['SCHWACH']:
            parts.append(f'SCHWACH: {", ".join(health["SCHWACH"])}')
        if parts:
            msg += f'\n📊 SCORING HEALTH\n  {" | ".join(parts)}\n'
