# Silver Hawk Portfolio - Single Source of Truth

**Letzte Aktualisierung:** 18.03.2026 (MU #2 bei +20% verkauft + BE! DAX SHORT geschlossen, DAX rallied nach FOMC)
**Startkapital März:** ~1.788 EUR (frisches Kapital eingezahlt)
**Ziel März:** +15% Baseline (2.056 EUR) / +30% Stretch (2.324 EUR) — v5 Strategie aktiv
**⚡ v5 aktiv ab 18.03.2026 (Live-Test)**

---

## Aktueller Stand

| Metrik | Wert |
|--------|------|
| **Portfolio-Wert** | **~1.588 EUR** (1.578 Cash + 9,97 NVIDIA) |
| **Investiert** | ~10 EUR (nur NVIDIA Bruchteil/TR-Geschenk) |
| **Cash frei** | **1.578 EUR (99%)** |
| **Offene Positionen** | **0** (nur NVIDIA Mini) |
| **März P&L (Trading)** | **~-300 EUR** (auf Startkapital 1.788€) |
| **Einzahlung** | +100 EUR (19.03, von Abdullahs Mutter für Tochter) |
| **Freie Slots** | **3 von 3** |

✅ **ENR.DE v5 Scout: Stop 1,83€ ausgelöst 19.03. Verlust -44,56€ (2,9% Ptf). v5 hat funktioniert: 60% Scout statt 100% = -45€ statt -89€!**
⚡ **v5 Strategie aktiv — erster Trade abgeschlossen, Stop hat geschützt**
⚠️ **Learning: Post-FOMC Makro > Technische Signale. SMA50-Reclaim hielt NICHT.**

---

## Offene Positionen (März)

| # | Symbol | Richtung | Stück | Buy-In | Aktuell | P&L | KO | Stop/Limit | Notiz |
|---|--------|----------|-------|--------|---------|-----|-----|------------|-------|
| ~~1~~ | ~~DAX SHORT Turbo KO 25.005~~ | ~~SHORT~~ | ~~12 Stk~~ | ~~246 EUR~~ | ~~VERKAUFT~~ | — | — | — | Stop 15,40€ ausgelöst 10.03 → siehe Geschlossene Trades |
| ~~2a~~ | ~~Gold LONG Turbo 50% (3 Stk)~~ | ~~LONG~~ | ~~3 Stk~~ | ~~32,39€~~ | ~~38,87€~~ | **~+19€** | — | — | ✅ 50% bei +20% verkauft (v3 Regel!) → siehe Geschlossene Trades |
| ~~2b~~ | ~~Gold LONG Runner (2 Stk)~~ | ~~LONG~~ | ~~2 Stk~~ | ~~32,39€~~ | ~~VERKAUFT~~ | **~+6€** | — | — | Runner verkauft 11.03, Position zu klein (~71€), Kapital frei für besseren Trade |
| ~~8~~ | ~~PLTR SHORT Turbo KO ~165$~~ | ~~SHORT~~ | ~~? Stk~~ | ~~~220 EUR~~ | ~~VERKAUFT~~ | — | — | — | Geschlossen 12.03 bei Break-Even (-2€). Slot frei für MU LONG → siehe Geschlossene Trades |
| 12a | ENR.DE LONG 50% (104 Stk) | LONG | 104 Stk | 1,915€/Stk | 2,41€/Stk | **+50,50 EUR (+25,9%)** | — | — | ✅ 50% bei +25% verkauft 17.03! v3 Kern-Strategie! → siehe Geschlossene Trades |
| ~~12b~~ | ~~ENR.DE LONG Runner (105 Stk)~~ | ~~LONG~~ | ~~105 Stk~~ | ~~1,915€/Stk (~201 EUR)~~ | ~~VERKAUFT~~ | **+59,40 EUR (+29,5%)** | — | — | ✅ Runner komplett verkauft 17.03! Gesamt ENR.DE: +109,90€ → siehe Geschlossene Trades |
| ~~11~~ | ~~DAX SHORT Turbo KO 25.003~~ | ~~SHORT~~ | ~~15 Stk~~ | ~~~242 EUR~~ | ~~VERKAUFT~~ | — | — | — | Geschlossen 18.03, DAX rallied nach FOMC → siehe Geschlossene Trades |
| 13a | MU LONG Turbo #2 (50%) | LONG | ? Stk | ~200 EUR | VERKAUFT | **+~20€ (+20%)** | — | Limit 1,00€ | ✅ 50% bei +20% verkauft 18.03! v3 Kern-Strategie zum 8. Mal! → siehe Geschlossene Trades |
| ~~13b~~ | ~~MU LONG Turbo #2 Runner~~ | ~~LONG~~ | ~~? Stk~~ | — | ~~VERKAUFT~~ | **~0€** | — | BE-Stop | BE-Stop ausgelöst 18.03. 0-Risiko Runner. v3 befolgt → siehe Geschlossene Trades |
| 10a | MU LONG 50% (180 Stk) | LONG | 180 Stk | 0,563€/Stk | 0,676€/Stk | **+~20 EUR (+20%)** | — | — | ✅ 50% bei +20% verkauft 13.03! v3 Regel befolgt! → siehe Geschlossene Trades |
| ~~10b~~ | ~~MU LONG Runner (180 Stk)~~ | ~~LONG~~ | ~~180 Stk~~ | ~~0,563€/Stk~~ | ~~VERKAUFT~~ | — | — | — | BE-Stop 0,563€ ausgelöst. ~0€ P&L. v3 Regel befolgt → siehe Geschlossene Trades |
| ~~9~~ | ~~TSM LONG Turbo KO ~$316~~ | ~~LONG~~ | ~~? Stk~~ | ~~250 EUR~~ | ~~200 EUR~~ | — | — | — | Stop ausgelöst 12.03, -50€ (-20%) → siehe Geschlossene Trades |
| ~~3~~ | ~~HIMS Aktie (kein Hebel)~~ | ~~LONG~~ | ~~7.14~~ | ~~151 EUR~~ | ~~151,36 EUR~~ | **+0,36 EUR** | — | — | Verkauft 17.03 bei Break-Even. Stop $21 war fast erreicht → siehe Geschlossene Trades |
| 4 | NVIDIA (Bruchteil) | LONG | 0.063 | ~11 EUR | ~10 EUR | ~-1 EUR | - | - | Mini-Position, irrelevant |
| ~~5~~ | ~~ENR.DE LONG 50% (65 Stk)~~ | ~~LONG~~ | ~~65 Stk~~ | ~~1,91€~~ | ~~2,46€~~ | **~+36€** | — | — | ✅ 50% bei +29% verkauft (v3 Regel!) → siehe Geschlossene Trades |
| ~~6~~ | ~~ENR.DE SHORT Turbo KO 160,07~~ | ~~SHORT~~ | ~~70 Stk~~ | ~~1,004€~~ | ~~~0,70€~~ | **~-21€** | — | — | Geschlossen, Geld in LONG umgeschichtet → siehe Geschlossene Trades |
| ~~7~~ | ~~ENR.DE LONG Runner KO 133,00~~ | ~~LONG~~ | ~~65 Stk~~ | ~~1,91€~~ | ~~VERKAUFT~~ | — | — | — | Trailing Stop BE 1,91€ ausgelöst 11.03 → siehe Geschlossene Trades |
| ~~5~~ | ~~ASTS LONG Turbo x75~~ | ~~LONG~~ | ~~? Stk~~ | ~~~164 EUR~~ | ~~VERKAUFT~~ | — | — | — | Break-Even Stop ausgelöst 09.03 → siehe Geschlossene Trades |
| ~~6~~ | ~~AMD LONG Turbo KO 174,39~~ | ~~LONG~~ | ~~185 Stk~~ | ~~439 EUR~~ | ~~VERKAUFT~~ | — | — | — | Verkauft 06.03 Freitag Abend, -26% (~-114€) → siehe Geschlossene Trades |
| ~~7~~ | ~~SI=F SHORT Turbo KO 97,10~~ | ~~SHORT~~ | ~~13 Stk~~ | ~~173 EUR~~ | ~~VERKAUFT~~ | — | — | — | Geschlossen 06.03, -18,5% → siehe Geschlossene Trades |

---

## Geschlossene Trades (März)

| Symbol | Kauf (€) | Verkauf (€) | P&L EUR | Notiz |
|--------|----------|-------------|---------|-------|
| TSM SHORT KO 405$ (71 Stk.) | ~211 (Feb) | 227,62 | **+16,62** | Stop-Sell 02.03 |
| SI=F LONG KO 82$ (42 Stk.) | 400,42 (27.Feb) | 395,90 | **-4,52** | Stop-Sell 02.03 |
| ENR.DE LONG KO 138.98€ | 458,70 | 285,35 + 165,75 = 451,10 | **-7,60** | Kauf 02.03, 50% Limit-Sell +20%, Rest 03.03 verkauft |
| GC=F LONG KO 5149$ (falsch) | 450,40 | 456,80 | **+6,40** | Falscher Turbo (KO=Stop), sofort korrigiert |
| GC=F LONG KO 4975$ (richtig) | 443,00 | 258,42 | **-184,58** | Iran-Krieg Volatilität, -41.7% auf Turbo |
| Zinsen 2% p.a. | — | 2,66 | **+2,66** | 01.03 |
| ENR.DE LONG KO 138,40 (50%) | ~275 (139 Stk) | ~330 | **~+55** | 50% bei +20% verkauft 05.03 ✅ |
| HOOD LONG KO 62,75 (50%) | ~82 (5 Stk) | ~98 | **~+16** | 50% bei +20% verkauft 05.03 ✅ |
| ENR.DE LONG KO 138,40 (Runner 139 Stk) | ~275 | ~275 | **~0** | Break-Even Stop ausgelöst 05.03 |
| DAX SHORT KO 25.009 (50%, ~9 Stk) | ~79 (Buy-in 8,80) | ~95 | **~+16** | 50% bei +20% verkauft 05.03 ✅ |
| HOOD LONG KO 62,75 (Runner 5 Stk) | ~82 | ~82 | **~0** | Break-Even Stop ausgelöst 05.03 |
| DAX SHORT KO 25.009 (Runner 10 Stk) | ~88 (Buy-in 8,80) | ~123 (12,32€) | **~+35** | Runner bei +40% verkauft 05.03 abends! 🔥 |
| SI=F SHORT KO 97,10 (13 Stk) | 173,12 | 141,09 | **-32,03** | Unter 60% Gate getradet (55%)! Learning: REGEL IST REGEL |
| AMD LONG KO 174,39 (185 Stk) | 439,45 | 304,83 (261,28+43,55) | **-134,62** | Verkauft 06.03 Fr Abend bei -30,6%. Doppelte Divergenz war da, aber AMD fiel weiter. Wochenend-Gap-Risiko zu hoch |
| ASTS LONG Turbo x75 | ~164 | ~164 | **~0** | Break-Even Stop ausgelöst 09.03. Score 7/7 aber x75 Hebel zu riskant → BE-Stop war richtig |
| DAX SHORT KO 25.005 (12 Stk) | 246 | 184,80 (15,40€×12) | **-61,20** | Stop 15,40€ ausgelöst 10.03. **EIGENMÄCHTIGER TRADE** — erster DAX SHORT war +40% Runner (perfekt!), dann OHNE neue Analyse nachgekauft aus Gier. Learning: Trade war durch, Geduld fehlte |
| ENR.DE SHORT KO 160,07 (70 Stk) | 70,28 | ~49 | **~-21** | Geschlossen 11.03 bei ~-30%. Geld in LONG umgeschichtet. Spread war erhöht beim Kauf |
| ENR.DE LONG 50% (65 Stk) | ~124 | ~160 (2,46€×65) | **~+36** | ✅ 50% bei +29% verkauft! Kern-Strategie v3 befolgt! Rest als Runner mit Trailing Stop BE |
| Gold LONG 50% (3 Stk) | ~97 (32,39€×3) | ~117 (38,87€×3) | **~+19** | ✅ 50% bei +20% verkauft! v3 Regel! |
| Gold LONG Runner (2 Stk) | ~65 (32,39€×2) | ~71 (~35,50€×2) | **~+6** | Runner verkauft 11.03 — Position zu klein (~71€), besser Capital redeployen |
| ENR.DE LONG Runner (65 Stk) | ~124 (1,91€×65) | ~124 (1,91€×65) | **~0** | Trailing Stop BE 1,91€ ausgelöst 11.03. 50% bereits bei +29% verkauft (+36€) |
| PLTR SHORT Turbo KO ~165$ | ~220 | ~218 | **-2** | Geschlossen 12.03 bei Break-Even. Daten-basierte Entscheidung: ADX 28,3 Uptrend, +DI 33 >> -DI 12, MACD bullish = SHORT kämpfte gegen starken Trend. Slot frei für MU LONG |
| MU LONG 50% (180 Stk) | ~101 (0,563€×180) | ~122 (0,676€×180) | **+~20** | ✅ 50% bei +20% verkauft 13.03! v3 Kern-Strategie! Rest 180 Stk Runner auf BE-Stop |
| TSM LONG Turbo KO ~$316 | 250 | 200 | **-50** | Stop $330 ausgelöst 12.03. -20% auf Turbo. SMA50 $343 hat nicht gehalten. Markt-Schwäche + Semi-Sektor unter Druck |
| MU LONG Runner (180 Stk) | ~101 (0,563€×180) | ~101 (0,563€×180) | **~0** | BE-Stop 0,563€ ausgelöst. v3 Regel befolgt — 0-Risiko Runner |
| HIMS Aktie (7,14 Stk) | 151 | 151,36 | **+0,36** | Verkauft 17.03 bei Break-Even. Stop $21 war fast erreicht |
| ENR.DE LONG 50% (104 Stk) KO 125,71 | ~199 (1,915€×104) | ~251 (2,41€×104) | **+50,50** | ✅ 50% bei +25% verkauft 17.03! v3 Kern-Strategie! Limit 2,41€ exakt gefüllt |
| ENR.DE LONG Runner (105 Stk) KO 125,71 | ~201 (1,915€×105) | ~260 (~2,48€×105) | **+59,40** | ✅ Runner bei +29,5% komplett verkauft 17.03! Gesamt ENR.DE: +109,90€ |
| MU LONG Turbo #2 (50%) | ~100 | ~120 | **+~20** | ✅ 50% bei +20% verkauft 18.03! MU Earnings Beat. v3 Kern-Strategie zum 8. Mal! |
| MU LONG Turbo #2 Runner | ~100 | ~100 | **~0** | BE-Stop ausgelöst 18.03. 0-Risiko Runner, v3 befolgt |
| DAX SHORT Turbo KO 25.003 (15 Stk) | ~242 | ~183 | **~-59** | Geschlossen 18.03. DAX rallied nach FOMC. Hedge-Funktion erfüllt während MU+HIMS liefen |
| ENR.DE LONG v5 Scout (76 Stk) KO 126,63 | 183,40 (2,41€×76) | 138,84 (1,83€×76) | **-44,56** | Stop 1,83€ ausgelöst 19.03. ERSTER v5 TRADE. Scout 60% = -45€ statt -89€ bei v3! SMA50-Reclaim hielt nicht, Makro (Post-FOMC + ECB) überrollte Technik. Learning: Post-FOMC 3-5 Tage warten! |
| **GESAMT** | | | **~-233** | Drawdown -16,8%. Komplett flat, 99% Cash. v5 Stop hat funktioniert! |

---

## Abgeschlossene Analysen (März)

| Datum | Symbol | Signal | Konfidenz | Ergebnis |
|-------|--------|--------|-----------|----------|
| 27.02 | TSM | SHORT HALTEN | 63% | Re-Analyse: Blow-Off-Top $390, RSI-Kollaps, Cover $360/$345/$332 |
| 27.02 | SI=F | LONG (TAKTISCH) | 65% | Entry $90-93, KO $81.69, Stop $86, Exits $100/$108/$120 |
| 27.02 | GC=F | LONG (KONSERVATIV) | 60% | Entry $5,180-5,220, KO $5,014, Stop $5,100, Exits $5,400/$5,500/$5,586 |
| 01.03 | FIG | SHORT | 68% | Entry $28.50-30, KO $36.36, Stop $33, Cover $25/$22/$20 |
| 02.03 | ENR.DE | LONG (TAKTISCH) | 64% | Entry EUR 155-159, KO EUR 140, Stop EUR 150. Gekauft, 50% bei +20% verkauft, Rest bei -33% geschlossen |
| 02.03 | GC=F | LONG (STARK) | 72% | Entry $5,300-5,350, KO $4,975, Stop $5,150. Gekauft (richtiger KO), aber Iran-Volatilität → -41.7% Verlust |
| 02.03 | HOOD | WARTEN | 58% | Unter 60% Gate. Death Cross, ATR 5.5% zu riskant. SHORT wenn $73 bricht, LONG wenn $80 bricht |
| 02.03 | GC=F | LONG Re-Analyse | 72% | Bestätigt. Analyse war richtig, aber Turbo-Hebel + Geopolitik = tödliche Kombi |
| 03.03 | ASML | WARTEN | 52% | Unter 60% Gate. RSI-Kollaps 82→49, Scorecard 31:30 Unentschieden |
| 03.03 | ENR.DE | WARTEN/HALTEN | 55% | Unter 60% Gate. Iran-Krieg drückt alle EU-Aktien. Q1 Rekord (17.6B Aufträge, Gewinn x2). RSI 38, kein Umkehrsignal |
| 03.03 | ^GDAXI | SHORT (TAKTISCH) | 72% | Entry 24.000-24.300 (Bounce), KO 25.100, Stop 24.600, Cover 23.000/22.500/22.000. Bearische Divergenz bestätigt, alle Supports gebrochen |
| 04.03 | RCAT | LONG (SPEKULATIV) | 62% | Entry $13.90-14.50 (Rücksetzer), Stop $11.20, Exits $16.70/$18/$20. NUR Lotto 10% OHNE Hebel! ATR 9.3%, Earnings 18.03 → vorher 50% raus! |
| 04.03 | ENR.DE | LONG (TAKTISCH) | 68% | Entry EUR 155-160, KO EUR 139, Stop EUR 148, Exits EUR 170 (50%)/180 (30%)/195+ (Rest). Q1 Rekord (17.6B Aufträge, Gewinn 3x, FCF 2x). Turbo möglich (ATR 3.5%), Hebel ~7.6x |
| 04.03 | ASML | WARTEN | 58% | Unter 60% Gate! KOSPI -12%, Semi-Sektor unter Druck, keine RSI-Divergenz. Entry-Trigger: RSI <35 + Divergenz ODER SMA50-Test ($1.322) + Hammer. KO $1.267, Stop $1.310 |
| 04.03 | HOOD | LONG (SPEKULATIV) | 61% | Knapp über Gate. BULLISCHE RSI-Divergenz ($69/$73 vs RSI 30/38), MACD dreht. NUR Lotto 10% OHNE Hebel! Stop $69, Exits $80 (50%)/$90 (30%)/$99+ (Rest). Death Cross + Beta 2,48 = hohes Risiko |
| 05.03 | AMD | LONG (TAKTISCH) | 66% | BULLISCHE DIVERGENZ ($194/RSI26→$188/RSI33), Doppelboden SMA200. KO $175, Stop $188, Exits $218 (50%)/$240 (30%)/$260+ (Rest). Meta 6GW Deal, PEG 0,55. Lotto 175€/Klein 440€ |
| 05.03 | TSM | WARTEN | 55% | UNTER 60% Gate! RSI-Kollaps 84→39 OHNE Divergenz. Fundamentals TOP (PEG 0,57, Goldman Upgrade) aber kein Umkehrsignal. Entry-Trigger: Divergenz ODER SMA50-Test ($337)+Hammer ODER Breakout $375. KO $300-310 |
| 05.03 | TSLA | WARTEN (LONG BIAS) | 55% | UNTER 60% Gate! RSI 42,7, ATR 2,8%, Scorecard 34:31 LONG (<5pt Differenz). Robotaxi/Optimus Katalysatoren, aber Musk-Risiko + Auto-Marge unter Druck. Entry-Trigger: RSI <35 + Divergenz ODER SMA200-Test ($348) + Hammer. KO $377, Stop $390 |
| 05.03 | ASML | WARTEN (LONG BIAS) | 57% | 3. Analyse März (52%→58%→57%). SMA50-Test ($1.329) läuft, ABER kein Hammer + keine RSI-Divergenz. RSI 42,8 (Kollaps 82→43), Scorecard 29:33 SHORT. ⚠️ Semi bei 25,6% (AMD) — nur Lotto/Klein! Entry-Trigger: RSI <35+Divergenz ODER SMA50-Hammer+grüne Kerze. KO $1.260, Stop $1.300 |
| 05.03 | ENR.DE | LONG (TAKTISCH) | 62% | 🔥 Stärkste Katalysatoren März! EUR 2B Buyback ab 04.03, Stoxx50 Aufnahme 23.03, JPMorgan BUY EUR 200. RSI 42 (Delta -10,4!), Slope beschleunigt — NICHT HEUTE kaufen! Entry bei RSI-Wende oder SMA50-Hammer. KO EUR 137,50, Stop EUR 145, Exits EUR 170/185/200. Klein 25-35% |
| 05.03 | GC=F | LONG (TAKTISCH) | 66% | 3. Gold-Analyse März! Rücksetzer auf $5.000-$5.050 ABWARTEN (Shooting Star heute). KO $4.770 (3x ATR!), Stop $4.900, Exits $5.300 (50%)/$5.500 (30%)/$5.600+ (Rest). Iran-Krieg Tag 6 + ZB-Käufe 60t/Mo. JPM $6.300, Goldman $5.400. Kein Divergenz-Signal. ⚠️ Letzter Trade -184€ wegen KO zu eng (1,38x ATR) → diesmal 3x ATR! Klein 25% (431€) |
| 05.03 | SI=F | WARTEN (LONG BIAS) | 55% | UNTER 60% Gate! Blow-Off-Top -32% ($121→$82), Preis UNTER SMA50 ($84,24), ATR 5,0% EXTREM, Ann.Vol 107%. JPM Q2-Ziel $75 (-9%). Solar-Substitution (Kupfer ab Q2). Gold/Silver Ratio 61,9 = überbewertet. Scorecard 30:35 SHORT. RSI-Momentum dreht positiv (+9,4), Doppelboden $79-80. Entry-Trigger: SMA50 Reclaim ODER RSI-Divergenz bei $79-80 |
| 06.03 | BE | WARTEN (LONG BIAS) | 57% | UNTER 60% Gate! ATR 8,9% EXTREM, Beta 3,18, Risk 10/10 — für Turbos untradebar. Fundamental TOP: $20B Backlog, $5B Brookfield-Deal, Q4 Beat (+21%), Guidance $3,1-3,3B (+55%). Aschenbrenner #1 Position (20,6%, $876M). Aber: P/E 55x, P/S 22x, Analyst Mean $143 < Preis $160. MACD bearish, keine Divergenz. NUR Lotto OHNE Hebel. Entry-Trigger: Rücksetzer $145-150 + Divergenz ODER Breakout >$168. KO $124, Stop $137 |
| 06.03 | CORZ | WARTEN | 52% | UNTER 60% Gate! Beta 6,9 ULTRA-EXTREM, ATR 6,7%. SMA200-Test ($15,78) = Make-or-Break. Scorecard 31:32 UNENTSCHIEDEN. Short Interest 19,3% (5,1d) = Squeeze-Potential. AI-Pivot: $850M Guidance 75-80% GM, CoreWeave 590MW. ABER: FCF -$539M, Schulden $1,66B, Revenue -38%, Accounting Weakness. Aschenbrenner 9,8% ($419M). LONG-Trigger: SMA200-Bounce + Divergenz. SHORT-Trigger: Break <$15,50. KO LONG $12,70, SHORT $18,70. NUR Lotto OHNE Hebel |
| 06.03 | WDC | WARTEN (LONG BIAS) | 55% | UNTER 60% Gate! RSI 39,5 FALLEND (-18,1/5d), KEINE Divergenz, ATR 7,1% SEHR HOCH. Scorecard 33:32 UNENTSCHIEDEN. Fundamentals TOP: NAND-Knappheit +33% QoQ, sold out CY26, $6B Buyback, GM 46,1%, Q2 Beat (EPS $2,13 vs $1,91). ABER: +970% Run → Korrektur, P/S 8,3x, Beta 1,85. Entry-Trigger: RSI-Divergenz <35 ODER SMA50 ($242)+Hammer ODER Breakout >$270+Vol. KO $218, Stop $235. Nur Lotto/Klein (ATR 7,1%!) |
| 06.03 | ^GDAXI | WARTEN | 52% | 2. DAX-Analyse März (03.03: SHORT 72% → +40% Runner!). Move gelaufen (-7,1%), RSI-MOMENTUM DREHT POSITIV (Slope -2,31→+1,76). Scorecard 32:33 UNENTSCHIEDEN. Unter SMA50 (-3,7%), MACD -187 tief bearish, ABER: €500B Fiskal + Auto-Exemption + ECB Zinssenkung möglich = mittelfristig bullish. SHORT-Trigger: Bounce 24.300-24.500 + Bearish Candle ODER Break unter 23.601. KO 25.100, Stop 24.700. DAX SHORT = guter HEDGE gegen AMD LONG |
| 09.03 | ^GDAXI | SHORT HALTEN | 72% | 3. DAX-Analyse März. Re-Analyse bestehende Position (KO 25.005, 12 Stk, -15%). DAX 23.202, Gap Down -2,5%. RSI 28,7 ÜBERVERKAUFT + FÄLLT WEITER (-6,3/Tag), KEINE Divergenz = Wasserfall intakt. Iran-Krieg eskaliert (US raus aus Saudi, Öl >$110). Industrieaufträge -11,1%. Scorecard 46:18 SHORT. KO 25.005 sicher (7,8% Abstand). Stop 15,40€ Cert (~23.465). Exits: +20% Cert → 50% raus, Trail BE, Stretch DAX 21.500. Time-Stop 13.03, vor ECB 19.03 sichern, bei Waffenstillstand SOFORT raus |
| 06.03 | ASTS | WARTEN (SHORT BIAS) | 58% | UNTER 60% Gate! ATR 8,3% EXTREM, Beta 2,87, Ann.Vol 127%, Risk 10/10. Blow-Off-Top: +13% (04.03) → -10,5% Reversal (05.03). P/S 503x ABSURD, FCF -$1,2B, Dilution 84% (148M→272M Shares). Short Interest 19,6% = SQUEEZE-RISIKO! Scorecard 28:41 SHORT klar. Analysten-Mean $89 < Preis $93,86. SHORT-Trigger: Bounce $100-105 + Bearish Candle. KO $113,36 (2,5x ATR), Stop $110, Targets $85/$77/$64. NUR Lotto OHNE Hebel! Für Turbos UNTRADEBAR (ATR 8,3%) |
| 06.03 | AMD | LONG HALTEN (Re-Analyse) | 66% | DOPPELTE BULLISCHE DIVERGENZ! Tiefs ($194/RSI26→$188/RSI33) + Hochs ($216/RSI38→$203/RSI45). SMA200 $188 hält. FwdPE 18,1x, Meta $100B Deal, Analyst Mean $290. Turbo ~1,85€ (-21,9%), Stop 1,66€, KO $174,39 (2,94x ATR). Time-Stop: 10.03 halbieren, 13.03 raus. Exits: $208/2,86€ (50%) → Trail BE → $218/$230+ |
| 06.03 | WIX | WARTEN (STRONG LONG BIAS) | 58% | UNTER 60% Gate! RSI 79,7 ÜBERKAUFT nach +54% V-Recovery ($60→$93). Death Cross intakt. ABER: Fundamentals TOP — FwdPE 11,4x, FCF Yield 11% ($573M), $2B Buyback (38% Marktcap!), ChatGPT Integration, Base44 $100M ARR. Analysten STRONG BUY, Mean $131 (+41%). SI 12,7% = Squeeze. Entry-Trigger: Pullback $83-87 + RSI <60. KO $80,90, Stop $82, Exits $100/$115/$127. Klein 25% (~414€) |
| 06.03 | GC=F | LONG (LOTTO) | 68% | 4. Gold-Analyse März! Double Bottom $5.023/5.055 hält, Bounce +1,7%. RSI 55 perfekt, Momentum dreht positiv. ATR 1,9% IDEAL für Turbos. KO $4.784 (3x ATR, unter SMA50+Puffer), Stop $4.950, Hebel ~13,6x. Lotto 10% (~157€). Exits: Cert +20% (50%)→$5.400 (30%)→$5.586+ (Rest). LEARNING: Letzter Trade -184€ wg. KO 1,38x ATR! Diesmal 3x ATR. JPM $6.300 EOY, Goldman $5.400. ⚠️ Entry MONTAG (Wochenend-Gap-Risiko) |
| 09.03 | GC=F | LONG HALTEN (VORSICHT) | 65% | 5. Gold-Analyse März! Re-Analyse bestehende Position (x14,84, 5 Stk, -13%). Gold $5.097, ATH-Korrektur -10% ($5.586→$5.021). RSI 55 Neutral, Momentum DREHT POSITIV (Slope -1,36→+0,89). Über SMA20 ($5.089) + SMA50 ($4.854), Golden Cross. DXY 99 = 4-Jahres-Tief bullish. Iran eskaliert (US raus aus Saudi). ⚠️ STOP-PROBLEM: Stop 22,67€ (~$5.033), heutiges Tief $5.021 = FAST AUSGELÖST! Stop = 0,65x ATR = ZU ENG! → Stop auf 20€ senken (~$4.920) ODER bei Bounce >$5.150 schließen + neu kaufen mit KO $4.751. CPI 12.03 + FOMC 17-18.03 = vorher sichern! Scorecard 39:24 LONG |
| 11.03 | ASML | WARTEN | 48% | 4. Analyse dieser Woche! UNTER 60% Gate! Scorecard 32:31 UNENTSCHIEDEN. RSI 43,8 (Reset von 82), KEINE Divergenz, MACD -22 bearisch. SMA50-Test ($1.346) = Make-or-Break. EUV-Monopol + TSMC $56B Capex vs China-Normalisierung (20% Revenue) + P/E 31,5x bei 5% Wachstum. 3/3 Slots belegt. LONG-Trigger: RSI-Divergenz ODER 3d über SMA50 ODER >$1.400. SHORT-Trigger: Break <SMA50 Closing. LONG KO $1.194, Stop $1.260, Exits $1.400/$1.475/$1.547. Earnings 15.04 |
| 12.03 | PNG.V | WARTEN (LONG BIAS) | 52% | UNTER 60% Gate! Kraken Robotics (CAD). C$9.96 (€6.34). ATR 7.4% = NUR ohne Hebel! P/E 142x, P/S 30x, FCF -C$34M. +420% in 1J parabolisch. Bearische Divergenz an 52W-Hochs. Analysten C$7.44 (25% unter Kurs!). C$615M Covelya-Akquisition (Offering C$350M schließt 12.03). Monopol Maritime Autonomous Systems, Revenue +60%, NATO-Tailwind. Scorecard 35:30 LONG. TSX-V = auf TR wahrsch. nicht handelbar. LONG-Trigger: Pullback C$7.50-8.50 + RSI<45. Earnings 27.04 |
| 12.03 | MU | WARTEN (STRONG LONG BIAS) | 58% | UNTER 60% Gate! $418.69 (€362). EARNINGS 18.03 IN 4 TAGEN = binäres Risiko! Forward P/E 8.87, HBM 2026 SOLD OUT, GM 68% guided, Rev +57%. RSI 50.4 perfekt neutral, V-Recovery von $358, SMA50 $383 hält. MACD bearish. KEINE Divergenz. ATR 5.0%. Wolfe $500, UBS $475, Mean $418. Scorecard 38:27 LONG klar. 3/3 Slots belegt + TSM LONG = Semi-Konzentration → max Klein 15%. POST-EARNINGS PLAN: Beat + Dip auf $380-400 = KAUFEN. KO $354, Stop $370, Exits $435/$455/$500+. |
| 12.03 | MU | LONG HALTEN (Re-Analyse) | 65% | Re-Analyse bestehende Position (360 Stk, -24%). $405.65 (€352). RSI 30.8 OVERSOLD (Cliff -22.4!), KEINE Divergenz. Regime TRANSITIONAL (ADX 22.5). Golden Cross intakt. FwdPE 8.5x, HBM sold out, GM 68%, 3 Upgrades $500-525. Scorecard 37:26 LONG. NSI +1.11 bullisch. KO $353.14 (12.9%, 2.6x ATR). Stop 0.27€ (~$385/SMA50). Exits: $435 (50%)/$455 (30%)/$500+ (Rest). ⚠️ PFLICHT: BE-Stop VOR Earnings 18.03 21:00! Duration: EVENT 1-2d. Drawdown -21.1% > 20% Limit! |
| 13.03 | MU | LONG HALTEN (Re-Analyse #3) | 65% | $405,35 (€352). RSI 52,1 Neutral (Recovery aus Oversold). **BULLISCHE DIVERGENZ** bei Tiefs ($374→$358/RSI 44→48)! Regime TRANSITIONAL (ADX 22,6). Golden Cross intakt, SMA50 $385 hält. FwdPE 8,4x, HBM4 sold out, Applied Materials Partnership 10.03, 24/0 Revisionen, GM 68% guided. Scorecard 41:22 LONG klar. NSI +1,17 stark bullisch. ATR 5,2%, ATR-Ratio 1,06x ✅. KO $353,14 (12,9%, 2,47x ATR). **BE-Stop 0,563€ spätestens 18.03 20:00 CET PFLICHT!** Exits: $435 (50%)/$455 (30%)/$500+ (Rest). FOMC 17.03 + Earnings 18.03 21:05 = doppeltes Event. Max. Verlust bei BE: ~48€ (3,4% Ptf). |
| 13.03 | ENR.DE | LONG (TAKTISCH) | 62% | EUR 145,00. RSI 21,3 EXTREM OVERSOLD, KEINE Divergenz (Wasserfall aktiv). Regime TRANSITIONAL (ADX 24,5). Golden Cross intakt, SMA50 EUR 147,84. ATR 5,2%, Beta 1,83. FwdPE 26,2x, PEG 0,48. Rekord-Q1: EUR 17,6B Aufträge, Backlog EUR 146B. EUR 2B Buyback läuft. **Stoxx50-Aufnahme 23.03 = Katalysator!** Scorecard 40:27 LONG klar. NSI +1,14 stark bullisch. Pre-Open LONG 64/100, Gap Fill 99%. KO EUR 126 (13,1%, 2,5x ATR). Stop EUR 135 (mental). Klein 15% empfohlen (~217€). Exits: Cert +20% (50%)/EUR 159 (30%)/EUR 166+ (Rest). Entry MONTAG nach XETRA-Open. Time-Stop 3d halbieren/5d raus. |
| 15.03 | ENR.DE | LONG HALTEN (Re-Analyse #9) | 57% | UNTER 60% Gate = KEIN Nachkaufen! EUR 143,95. RSI 40,9, Cliff -7,0/Tag, Slope -3,85 NEGATIV. SMA50 EUR 148 GEBROCHEN! MACD -2,58 bearisch. Bullische Divergenz NUR bei Hochs (schwach). Regime TRANSITIONAL (-DI 35,7 >> +DI 19,7). ATR 5,3%, Ratio 1,02x ✅. FwdPE 26x, PEG 0,48, FCF EUR 3,7B, Netto-Cash EUR 8B. Scorecard 34:29 LONG (knapp). NSI +0,53. Pre-Open KEIN TRADE (54/100). Bestehende Position: Turbo KO 125,71, 209 Stk, 1,915€, ~-5,5%. KO-Abstand 12,7% (2,4x ATR) ✅. Stop 1,33€ (EUR 139) BEIBEHALTEN — nur 3,4% Puffer! Exits: 2,30€ (+20%, 104 Stk)/2,68€ (63 Stk)/3,06€ (Rest). **KRITISCH: EUR 135,70 (V-Bottom) MUSS halten! Wenn bricht → SOFORT schließen!** Time-Stops: FOMC 18.03 hawkisch → 50% raus, Fr 21.03 kein Bounce >148 → 50%, 23.03 kein Stoxx50-Impuls → RAUS. Catalyst: Stoxx50 Aufnahme 23.03. Max. Verlust Stop: 122€ (8,4% Ptf). Drawdown -19% ALARMZONE! |
| 16.03 | GC=F | WARTEN | 40% | UNTER 60% Gate! $5.004, intraday $4.970 (unter $5.000!). RSI 46,8 Neutral, fallend, KEINE Divergenz. MACD bearisch. Regime RANGE (ADX 12,1, BB-Squeeze 2%!). Scorecard 30:28 LONG (Unentschieden). 4 rote Kerzen in Folge. DXY 100,35 (3,5-Mon-Hoch) + 10Y 4,28% = Gegenwind. Gold fällt TROTZ Iran-Krieg = Safe-Haven versagt kurzfristig. GLD $2,91B Abfluss (größter seit 2016). ZB-Käufe 190t/Q stützen langfristig. JPM $6.300 EOY. ⛔ V3 VETO: 4/3 Positionen offen. FOMC morgen. Drawdown -19% ALARMZONE. LONG-Trigger: SMA50-Bounce + Divergenz ODER FOMC dovish + Close >$5.100. KO $4.356 (Chart), Stop $4.900. SHORT-Trigger: Close <SMA50 ($4.918) + FOMC hawkish. |
| 17.03 | ENR.DE | LONG HALTEN (Re-Analyse #10) | 53% | UNTER 60% Gate = KEIN Nachkaufen! EUR 146,85 (+2,0%). Intraday-Rally 142,40→150,10 (+4,8%), Close 146,85 = SMA50-Rejection (langer Docht). RSI 43,8, +2,9/Tag, Slope -1,93 (verbessert von -3,85). Bullische Divergenz bei Hochs (schwach). MACD -2,55 bearisch. Regime TRANSITIONAL (ADX 24,8, -DI 33 >> +DI 18). ATR 5,3%, Ratio 0,85x ✅. FwdPE 26,5x, PEG 0,48, FCF EUR 3,7B, Netto-Cash EUR 8B. Scorecard 38:26 LONG. NSI +0,79. Pre-Open KEIN TRADE (53/100). Bestehende Position: Turbo KO 125,71, 209 Stk, 1,915€, ~+19%. KO-Abstand 14,4% (2,7x ATR) ✅. **⚠️ V3-ALARM: +29% erreicht, Limit +30% nicht getriggert — v3 sagt 50% bei +20%! SOFORT 104 Stk verkaufen!** Rest 105 Stk Trail BE 1,915€. Runner: 2,68€/3,06€. **TENDENZ nach grünem Tag: Historisch JEDE Rally verkauft (04.03: +6,4%→-6,6%, 09.03: +8,9%→4d Decline). KURZFRISTIG Rücksetzer 143-145, MITTELFRISTIG Stoxx50 23.03 = Aufwärts.** FOMC 18.03 = Risiko. Time-Stop: Fr 21.03 kein >148 → 50%, 23.03 kein Impuls → RAUS. |
| 17.03 | MU | LONG WARTEN (Post-Earnings) | 32%/67% | Re-Analyse bei $455,30 (52W-HOCH!). BB 103% = Breakout über Upper Band! RSI 63,2, Delta +11,1/5d EXPLOSIV, Slope +3,69 beschleunigend. MACD +3,26 bullish. KEINE Divergenz. Regime TRANSITIONAL (+DI 30 >> -DI 17). ATR 4,5%, Ratio 0,77x ✅ (Vol sinkt!). FwdPE 8,0x, Rev +57%, EPS +175%, HBM SOLD OUT 2026, GM 68%. Scorecard **44:20 LONG** (stärkste im März!). NSI +1,57 stark bullisch. Susquehanna $525, 39 Analysten BUY. Risk 3/10. NICHT VOR EARNINGS KAUFEN (32%)! FOMC 18.03 19:00 + Earnings 21:05 = Doppel-Event. POST-EARNINGS PLAN: Beat+Dip $420-445 = KAUFEN (67%). Beat+Guide Up >$10 Q3 = SOFORT 19.03. KO $375 (Chart, 2,7x ATR). Stop $395 (SMA50). Klein 15% (~234€). Exits: Cert +20% (50%)/$500 (30%)/$525+. Miss = KEIN ENTRY. First-Hour Dip 79% Win! |
| 17.03 | ENR.DE | LONG HALTEN (Re-Analyse #11) | 57% | Volle 4-Schritt-Analyse. UNTER 60% Gate = KEIN Nachkaufen! EUR 146,85. RSI 43,8, Delta +2,9/Tag, Slope -1,93 (verbessert). Bullische Divergenz Hochs (schwach). MACD -2,55 bearisch. Regime TRANSITIONAL. ATR 5,3%, Ratio 0,85x ✅. Scorecard 38:28 LONG. NSI +1,03 stark bullisch. Pre-Open KEIN TRADE (53/100, LONG Hit 49%). Dreifache SMA50-Rejection im März = stärkstes Bear-Argument. Katalysatoren: Triple-Index-Aufnahme 23.03 (5 Tage!), EUR 2B Buyback (820K Aktien gekauft), JPM EUR 200, UBS EUR 175, DB EUR 170. Bestehende Position: Turbo KO 125,71, 209 Stk, 1,915€. KO-Abstand 14,4% (2,7x ATR) ✅. **V3: Bei Cert 2,30€ (+20%) SOFORT 104 Stk verkaufen! Rest Trail BE 1,915€.** Exits: 2,30€/2,68€/3,06€. Stop 1,33€ (EUR 139). Time-Stop: Fr 21.03 kein >148 → 50%, 23.03 kein Impuls → RAUS. Max. Verlust Stop: 122€ (8,4% Ptf). |
| 17.03 | NBIS | WARTEN (⛔ VETO) | 44% | Nebius Group. $116,33 (€101). **HEUTE -10,4%** wegen $3,75B Convertible Debt Offering. ⛔ **VETO V2: CHOPPY + Score 30/60 <50.** ATR 6,1%, Vol 107%, Risk 10/10 = KEIN TURBO! Regime CHOPPY (ADX 16,8, BB-Width 98%). RSI 58,5 (Crash von 68,5, Delta -9,9/1d!). MACD +2,68 noch bullisch. Golden Cross. Short Interest **20,1%** (3,1d) = Squeeze-Potential! Revenue +501%, Target **$7-9B** (von $530M). Meta **$27B** Deal. NVIDIA **$2B** Investment. FCF **-$3,6B** Cash-Burn. Schulden $4,9B→$8,6B. P/S 55,6x, Forward P/E NEGATIV. Scorecard 30:31 UNENTSCHIEDEN. Analyst Mean $155 (+33%), High $291 (+150%). NUR Aktie ohne Hebel! Trigger: 3d stabil >$115 + RSI >55 (~55%) ODER SMA50 $97 + Divergenz (~62%). Max Lotto 5% (~78€). Earnings 29.04. |
| 13.03 | VST | WARTEN (LONG BIAS) | 58% | UNTER 60% Gate! $160.60 (€140.32). Death Cross aktiv (SMA50 $163 < SMA200 $182). RSI 44.9 Neutral, Slope DREHT POSITIV (-0.63→+0.99). **DOPPELTE BEARISCHE DIVERGENZ** (Tiefs+Hochs)! Regime RANGE (ADX 19.2, BB-Squeeze 12%). ATR 5.0%, Ratio 0.88x ✅. FwdPE 14.2x, EBITDA Guide $6.8-7.6B (+22%), Nuclear PPAs Amazon 3.8GW+Meta. Scorecard 34:30 LONG knapp. NSI +0.14 neutral. Insider: CEO $160M verkauft in 6 Mon! Pre-Open WAIT, Gap Fill 98%. KO $136.62 (Chart, 14.9%, 3.0x ATR). Stop $155. **LONG-Trigger: Tagesschluss >$163 (SMA50) + RSI>50.** SHORT-Trigger: <$155. Lotto/Klein 5-15% (~72-217€). Exits: Cert +20% (50%)/$182 (30%)/$200+ (Rest). NICHT vor FOMC 17.03! |
| 12.03 | HGRAF | KEIN TRADE (UNTRADEBAR) | 30% | HydroGraph Clean Power. $7.34 (€6.35). ATR 12.2% EXTREM = untradebar! P/S 40.700x auf $100K Revenue = absurd. RSI 77.7 überkauft + fällt (-11.3/Tag). +5.480% in 1J Penny Stock. OTC = nicht auf TR. C$30M Offering frisch. Graphen-Technologie langfristig interessant, aber Bewertung + Volatilität = für Silver Hawk System ungeeignet. Scorecard 17:36 SHORT (aber SHORT genauso untradebar). |
| 10.03 | SI=F | LONG (TAKTISCH) | 62% | V-Recovery nach CME-Crash ($121→$68→$89). RSI 64.9 gesund (SLV-Proxy), MACD bullisch, über SMA20+SMA50. 5. Jahr Angebotsdefizit (67M oz), DXY 99 (4J-Tief), Iran Safe Haven. Scorecard 40:26 LONG klar. ⚠️ NUR LOTTO (5%, ~74€): Gold LONG offen = 34% Edelmetalle, Ann.Vol 108% EXTREM, CME Margin-Risiko ab $100+. KO $76.47 (3x ATR Rohstoff, 14.5%). Stop $80. Exits: Cert +20% (50%)/$95 (30%)/$100 (Rest). ENTRY ABWARTEN: Pullback $85-87 (SMA50). Vor FOMC 17.03 sichern! Max. Verlust 42€ (2.8%) |
| 17.03 | SI=F | WARTEN (LONG BIAS) | 48% | UNTER 60% Gate! $80,72 (€70). UNTER SMA50 $86 (-6,3%). RSI 45 (SLV), Delta -8,4/5d FALLEND, Slope -1,35 (verbessert). KEINE Divergenz. MACD -0,54 bearisch. Regime **RANGE** (ADX 10,4, **BB-Squeeze 0%** = EXTREMSTE Kompression!). ATR 5,9%, Ratio 0,66x (Vol sinkt!). Ann.Vol 107% EXTREM. -DI 28,4 > +DI 24,8 (Bären leicht). Gold/Silver 62,1 normal. DXY 99,57 stark. 6. Jahr Defizit 67-100M oz, China Export Controls (44 Firmen), COMEX -70% seit 2020. JPM $81/Citi $150. Scorecard 33:26 LONG. NSI +0,79. LONG-Trigger: Close >$86 (SMA50)+Vol ODER FOMC dovish+>$83+RSI>50 → 60-62%. KO $67 (Chart, 3x ATR Rohstoff). Stop $76 (BB Lower). NUR Lotto 5% (~78€). Exits: Cert +20% (50%)/$93 BB Upper (30%)/$100+ (Rest). ⚠️ 2x SI=F Verlust! Ann.Vol 107% = nur Mini! |
| 10.03 | PLTR | SHORT | 68% | Death Cross + SMA50-Rejection ($156.67 exakt abgeprallt!). P/E 241x = teuerste Aktie S&P500. Pentagon Maven-Krise: Anthropic/Claude verbannt, Plattform-Umbau dauert Monate, OpenAI drängt als Ersatz. Scorecard 27:42 SHORT klar. RSI 69.7 (Kollaps von 79.5, Delta -9.8/Tag!), KEINE Divergenz. MACD noch bullish (Lagging nach V-Bounce). Short Interest nur 2.4% = kein Squeeze-Risiko. KO $164.16 (2.0x ATR, über SMA200 $162.22). Stop $161. Exits: Cert +20% (50%)/$138 (30%)/$126 (Rest). Klein 15% (~223€). Time-Stop 5d. Perfekter Hedge zu 3 LONGs. ATR 4.0%, ATR(5)/ATR(14) 1.16x ✅. Earnings 04.05 (weit weg). ⚠️ Vor FOMC 17.03 sichern |
| 10.03 | ENR.DE | LONG (WARTEN) | 62% | 7. Analyse März! V-Reversal 135,70→148,75 (+9,6%). RSI 38,9, Slope -4,49 NEGATIV, KEINE Divergenz. Fundamentals TOP: Backlog EUR 146B, FCF EUR 3,7B, EUR 2B Buyback. Stoxx50 Aufnahme 23.03 = Katalysator. JPMorgan EUR 200. ⚠️ ATR(5) 7,2% → NUR Klein/Ohne Hebel. NICHT VOR CPI 12.03 einsteigen! Entry-Signal: RSI >45 oder SMA50+Hammer. KO EUR 131 (2,5x ATR), Stop EUR 137, Exits EUR 160/170/180+. Klein 15% (~216€) |
| 11.03 | MDB | WARTEN (LONG BIAS) | 57% | Re-Analyse (07.03: LONG 63% → WARTEN 57%). RSI 27,2 OVERSOLD + FÄLLT WEITER (-3,2/Tag)! KEINE Divergenz. ADX 62,9 EXTREM bearish. MACD -29, kein Crossover. ATR 7,0% Grenzfall (ATR(5) 5,3% = sinkt). Scorecard 34:33 UNENTSCHIEDEN. Fundamentals TOP ($2,4B Cash, FCF $475M, Atlas 30% YoY), aber Technik NICHT bereit. Entry-Trigger: Bullische RSI-Divergenz ODER MACD-Crossover ODER Close >SMA200 $307. Wenn Trigger: Lotto 10% (148€), KO $223 (2,5x ATR), Stop $248, Exits $307 (50%)/$325 (30%)/$345+ (Rest). Vor FOMC 17.03 keinen Trade. Earnings 04.06 (weit weg) |
| 07.03 | MDB | LONG (TAKTISCH) | 63% | Post-Earnings Dip Buy! Q4 Beat (EPS $1,65 vs $1,46, Rev $695M), Crash -22% wg. FY27 Guidance ~17% Growth. RSI 28 OVERSOLD + Momentum dreht. $2,4B Cash/$62M Debt = FESTUNG. Analyst BUY $359 (+33%). Entry $258-270, Stop $245, KO $219 (2,5x ATR). Exits $307 (50%)/$325 (30%)/$340+ (Rest). Klein 25% (~394€). ⚠️ KEINE RSI-Divergenz, ATR 7,6% HOCH |
| 07.03 | HIMS | LONG (SPEKULATIV) | 62% | 🩳 SHORT SQUEEZE SETUP! 44,1% Short Float (81M Shares!), 2,7d DTC. Novo Nordisk Partnership = Game-Changer (von Klage zu Deal). +40% Aftermarket. NUR AKTIE, KEIN TURBO (ATR 7,8%, Beta 2,6)! Entry $20-22 (Rücksetzer nach Gap-Open Mo), Stop $17, Exits $26-27 (50%)/$30-32 (30%)/$35+ (Rest). Lotto 10% (~158€). FwdPE 10,5x, Rev Growth 28%, Gross Margin 73,8%. RSI 45,7 (Recovery von 4,5!), Bullische Divergenz. Earnings 04.05 |
| 06.03 | QBTS | WARTEN (LONG BIAS) | 55% | UNTER 60% Gate! ATR 7,0% EXTREM, Vol 102%, Beta 1,66 — für Turbos UNTRADEBAR. P/S 292x bei nur $24,6M Rev (FY25 +179%). Q4 Rev Miss -26% ($2,75M vs $3,72M). ABER: BULLISCHE DIVERGENZ ($17,46→$17,26/RSI 39→50). Bookings-Explosion: $30M allein Jan 2026! $884M Cash, $43M Debt = 15J Runway. Analyst STRONG BUY, Mean $38 (+96%). Scorecard 30:31 UNENTSCHIEDEN. NUR Lotto 10% OHNE Hebel. Entry-Trigger: SMA200 Reclaim $22,46+RSI>55 ODER Double Bottom $17 bestätigt. Earnings 07.05 |
| 11.03 | AXON | SHORT | 64% | Momentum-Breakdown nach V-Recovery $396→$588. RSI-Kollaps 83→69 (-8,2/Tag!). Death Cross aktiv (SMA50 $537 unter SMA200 $665). P/E 358x ABSURD, Operating Margin -3,1%. Scorecard 26:38 SHORT klar. ATR 5,3%, ATR(5)/ATR(14) 0,88x ✅. Korrelation mit PLTR SHORT 0,58 = MITTEL → NUR Lotto 10% (148€)! KO $600,15 (2,5x ATR, 13,2%). Stop $590. Exits: $495 (50%)/$470 (30%)/$440 (Rest). CPI heute + FOMC 17.03 = vor FOMC sichern! Earnings 06.05. Short Interest nur 3,7% = kein Squeeze-Risiko. Entry bei Bounce auf SMA50 $537 oder sofort |
| 11.03 | TSM | LONG (TAKTISCH) | 63% | Re-Analyse (05.03: WARTEN 55% → LONG 63%). SMA50 $343 hält als Support, Hammer 09.03 bei 19,2M Vol. RSI 42,3, Slope +0,47 positiv, KEINE Divergenz. Revenue Jan-Feb +30% YoY (AI-Supercycle). Goldman Conviction Buy NT$2.330. FwdPE 19,3x, Gross Margin 59,9%. MACD bearisch (-4,74), ADX 18,6 (RANGING). Scorecard 37:29 LONG klar. ATR 3,1% IDEAL für Turbos, ATR(5)/ATR(14) 1,14x ✅. KO $316 (Chart-basiert, unter S2 $319). Stop $330. Klein 15% (~222€). Exits: $375 (50%)/$390 (30%)/$410+ (Rest). Ex-Div 17.03 ($0,76). ⚠️ Vor FOMC 17-18.03 min. 50% sichern! Earnings 16.04 (weit weg). Max. Verlust ~43€ (2,9%). Alternativ WARTEN auf bullische Divergenz → dann Standard 35% |
| 11.03 | PLTR | SHORT HALTEN (Re-Analyse) | 66% | 2. PLTR-Analyse März (10.03: SHORT 68% → HALTEN 66%). SHORT-These INTAKT: **DREIFACHE SMA50-Rejection** (06/09/10.03) = stärkstes bearisches Chartsignal! Death Cross weitet sich: Gap -$0.43→-$7.44 in 2 Wochen. RSI 70.2 ÜBERKAUFT in Death-Cross = historisch Verkaufssignal. Maven-Krise ungelöst: Hegseth fordert sofortige Anthropic-Trennung, Anthropic verklagt Pentagon (Richterin MI 12.03). P/E 242x teuerste S&P500-Aktie. Scorecard 29:42 SHORT klar. MACD bullish (Crossover +3.12) = einziges Bull-Signal, aber Lagging. Short Interest 2.4% = kein Squeeze-Risiko. Turbo ~-13% (~-29€), Basiswert nahe Entry $152.41. Stop $159 (angepasst von $161). Exits: $145 (50%)/$138 (30%)/$126 (Rest). Time-Stop: 15.03 Check, 17.03 halbieren. ⚠️ MI 12.03 Anthropic-Richterin: wenn gewinnt → SOFORT schließen! Vor FOMC 17-18.03 min. 50% sichern. KO ~$165 (8.3% Abstand, 2.1x ATR) ✅ |
| 17.03 | PLTR | WARTEN | 45%/56% | 3. PLTR-Analyse März! Situation VERÄNDERT seit SHORT 66%! $154,98 (€134). **MACD +1,81 BULLISH CROSSOVER** (war bearish!). +DI 27,8 > -DI 19,1 (Bullen gewonnen). Preis ÜBER SMA50 $153 (+1,1%). ABER: **Death Cross AKTIV** (SMA50 $153 < SMA200 $163). P/E 246x, Risk 10/10 MAXIMUM. RSI 57,4 neutral, verbessert. ATR 3,8% ideal. Pre-Open **SHORT 76/100** (84% FH-Dip Win) — widerspricht MACD! Scorecard 31:33 UNENTSCHIEDEN. NSI +0,93. Maven $1,3B (+$795M), NATO MSS, $1,5B London HQ, $10B DoD Gerücht. **SMA200 $163 = DER KEY-LEVEL!** SHORT-Trigger: SMA200-Rejection $162-163 + Bear-Kerze → 62%. KO $175, Stop $168. LONG-Trigger: 3d über $163 → 63%. KO $147, Stop $152. NUR Lotto 5% (~78€) wegen Risk 10/10! Earnings 04.05. FOMC morgen. |
| 12.03 | HIMS | LONG HALTEN (Re-Analyse) | 65% | 2. HIMS-Analyse März! V-Recovery nach Novo-Deal ($14→$25, +77%). RSI 77 ÜBERKAUFT + dreht runter (-4,2/Tag), Slope-Einbruch +11,85→+1,36. KEINE Divergenz. ADX 60,9 STARKER Trend. MACD Bullish Crossover. 46,1% Short Float, Squeeze Score 100/100. Novo Deal: Ozempic+Wegovy auf Plattform, Klage vom Tisch. Scorecard 40:27 LONG klar. ATR 7,5% = NUR Aktie! Stop von $17 auf $21 anheben (Gap-Fill). Exits: $27 (50%)/$30 (30%)/$35+ (Rest). Time-Stop 5d unter $24 → halbieren. Earnings 04.05 weit weg. Max. Verlust ~22€ (1,6%). KEIN Nachkaufen bei RSI 77! |
| 12.03 | ^GDAXI | SHORT (HEDGE) | 70% | 4. DAX-Analyse März! DAX 23.539, -7,7% vom ATH. RSI 29,9 OVERSOLD + FÄLLT WEITER (-4,2/Tag), KEINE Divergenz = Wasserfall intakt. MACD -332, -DI 42,7 >> +DI 15,5, DX 46,8 STARK. ATR 2,0% IDEAL für Turbos. Scorecard 22:44 SHORT klar. KO 25.004 (über SMA50 + 1% Puffer, 6,2%). Stop 24.300 (mental). Iran-Krieg = Haupttreiber. v3 HEDGE zu MU LONG + HIMS LONG. Lotto 10% (~141€). Exits: Cert +20% (50%)/22.500 (30%)/22.000 (Rest). TAKTIK: ECB-Bounce morgen 13.03 auf 24.000 abwarten = idealer Entry. Time-Stop 5d. Vor FOMC 17.03 sichern. Bei Waffenstillstand SOFORT raus! |

---

## Anstehende Events

| Datum | Event | Relevanz | Aktion |
|-------|-------|----------|--------|
| LAUFEND | US-Iran Krieg | SEHR HOCH | Risk-Off, Safe Haven dominiert |
| 11.03 | CPI Februar (2,4% YoY, +0,3% MoM) | Hoch | Leicht über Fed-Ziel, Energie +0,6% |
| **16.03** | **DXY 100,35 (3,5-Mon-Hoch), 10Y 4,28%** | **HOCH** | **Gold unter Druck, Dollar-Stärke** |
| 10.03 | APLD Earnings | Hoch | Beobachten |
| **19.03** | **ECB Zinsentscheidung (Polymarket: 99% HOLD bei 2,65%)** | **HOCH** | **Kein Bounce durch Zinssenkung erwartet! DAX SHORT intakt** |
| **18.03** | **MU Earnings (nach Börsenschluss 21:05 CET)** | **SEHR HOCH** | **BE-Stop VOR Earnings! 55min Window bis TR-Schluss 22:00** |
| 17-18.03 | FOMC Meeting | Hoch | Fed erwartet HOLD, aber Inflation durch Öl? |
| 15.04 | ASML Earnings | Mittel | Weit weg |
| 16.04 | TSM Earnings | Mittel | Weit weg |
| 29.04 | HOOD Earnings | Mittel | Vor Earnings min. 50% sichern! |
| 05.05 | AMD Earnings | Mittel | Vor Earnings min. 50% sichern! |
| 06.05 | WDC Earnings Q3 FY26 | Mittel | Weit weg, Q3 Guidance $3,2B |
| 04.05 | HIMS Earnings Q1 2026 | Mittel | EPS $0,13 erwartet, Rev $616M |
| 07.05 | QBTS Earnings Q1 | Mittel | Weit weg, EPS -$0,08 erwartet |
| 11.05 | ASTS Earnings Q1 | Mittel | Weit weg, EPS -$0,17 erwartet |
| 06.05 | AXON Earnings Q1 | Mittel | EPS $1,60 erwartet, Rev $779M. Vor Earnings min. 50% sichern! |
| 04.06 | MDB Earnings Q1 FY27 | Mittel | Weit weg, EPS $1,18 erwartet, Rev $664M |
| 12.05 | ENR.DE Earnings Q2 | Mittel | Weit weg |

---

## Learnings

### Februar (wichtigste)
- NVDA Turbo war nicht im Tracking → ALLE Positionen erfassen!
- NVDA Turbo hatte keinen Stop → IMMER Stop beim Kauf!
- HOOD Stop hat -57 EUR gerettet statt Totalverlust → STOPS FUNKTIONIEREN!
- Turbo mit 7.9x Hebel: Stock -4% = Turbo -32% → Stop ist ÜBERLEBENSWICHTIG
- 0 EUR Cash = 0 Handlungsfreiheit
- Kern-Strategie funktioniert — SI=F (+10%), NVO (+20%), VST (+50%) alle durch Stops realisiert
- Portfolio-Screenshots sind IMMER aktueller als Memory-Datei!

### März (NEU)
- **Gold LONG war RICHTIG (72% Konfidenz)** — Richtung stimmte, war im Plus, aber +20% NICHT mitgenommen!
- **Bei +20% SOFORT 50% raus — GILT AUCH für Turbos!** — Kern-Strategie befolgen, immer!
- **Falschen Turbo gekauft (KO 5149 = unser STOP-Level!)** — IMMER KO prüfen vor Kauf! KO ≠ Stop!
- **ENR.DE Teilverkauf bei +20% war richtig** — 285 EUR gesichert statt komplettem Verlust
- **ENR.DE Q1 Rekord-Zahlen, trotzdem -5.5%** — Makro schlägt Fundamentals kurzfristig
- **Geopolitik-Trades können funktionieren** — Fr→Mo Gold-Call war goldrichtig, Ausführung war das Problem
- **UNTER 60% Gate = KEIN TRADE, auch kein "Lotto"!** — SI=F SHORT 55% Konfidenz, -16% über Nacht. Regel ist Regel.
- **Bei ATR >5% + Safe Haven KEINE Overnight-SHORTs** — Silber + Iran-Krieg = Hoch-Druck, SHORT über Nacht = Selbstmord
- **NIEMALS Trades ohne 4-Schritt-Analyse!** — DAX SHORT Freitag Nacht = Lotterie. These mag stimmen, aber ohne Analyse kein kalkuliertes Risiko
- **MAX 3 Positionen EINHALTEN!** — 5 Positionen = 0 Cash = 0 Kontrolle. Februar-Fehler wiederholt
- **Break-Even Stop bei Gewinnern = RICHTIG** — ASTS +8% → BE-Stop ausgelöst bei 0%. Besser 0 als Minus
- **Nahe -20% Drawdown = ALARMZONE** — Bei -17,9% im März. Nächster Fehler triggert 24h Pause
- **GIER NACH ABGESCHLOSSENEM TRADE** — DAX SHORT war +40% Runner (perfekt!). Dann nachgekauft weil "da geht noch mehr". -61€ Verlust. Wenn ein Trade durch ist, ist er DURCH. Geduld auf neues Setup warten!
- **US-Aktien Turbos IMMER erst nach US-Open (15:30 CET) kaufen!** — Vor 15:30 basiert der Turbo-Preis auf Market-Maker-Pricing mit dünner Liquidität, nicht auf echter Preisfindung. MU bei Max gekauft weil vor US-Open eingestiegen → sofort -20%

---

## Archiv Februar

**Bilanz:** 957 EUR → 753 EUR (-21.3%). Ziel 1.914 EUR nicht erreicht.

<details>
<summary>Geschlossene Trades (Februar)</summary>

| Symbol | P&L EUR | Notiz |
|--------|---------|-------|
| AAPL (HT7817) Teilverkauf | +500 | RSI 81, Teilverkauf 215x |
| MSTR SHORT | +24 | |
| QBTS LONG | -99 | D-Wave, waren +30% im Plus! |
| SI=F SHORT (alt) | -26 | |
| SI=F LONG (alt) | -17 | |
| SI=F LONG (12.02) | -29 | Blow-Off-Top, KO zu eng |
| NVDA Turbo (13.02) | ~-75? | Verkauft, 4 rote Tage |
| ARM Turbo (18.02) | +31 | 84x verkauft, Erlös ~244 EUR |
| IREN Turbo (13.02) | ~-20? | Verkauft, KO war zu eng |
| AMD Turbo | ? | Verkauft, Zeitpunkt/P&L unbekannt |
| HOOD Turbo (17.02) | -57 | Stop bei ~6.44 EUR ausgelöst |
| VST Teilverkauf (50 Stk.) | ~+89? | 50 Stk. verkauft, Buy-In 3.12 → ~4.90 |
| RKLB Turbo | ? | Verkauft, P&L unbekannt |
| SI=F LONG (neu, 80 Stk.) | ~+12 EUR | Stop bei +10% (1.71 EUR) ausgelöst |
| NVO SHORT (6 Stk.) | ~+9 EUR | Stop bei +20% ausgelöst — Kern-Strategie! |
| VST LONG (letzte 50 Stk.) | ~+78 EUR | Stop bei +50% (~4.68 EUR) ausgelöst 26.02 |
| ENR.DE LONG (135 Stk.) | ~+3 EUR | Stop bei ~Break-Even (~2.10 EUR) ausgelöst 26.02 |

</details>

<details>
<summary>Abgeschlossene Analysen (Februar)</summary>

| Datum | Symbol | Signal | Konfidenz | Ergebnis |
|-------|--------|--------|-----------|----------|
| 12.02 | SI=F | SELL/SHORT | 82% | LONG geschlossen, SHORT eröffnet |
| 13.02 | ARM | HOLD/Teilverkauf | 58% | Verkauft 18.02 |
| 13.02 | AMD | LONG (WARTEN) | 65% | Gekauft, dann verkauft |
| 13.02 | SI=F | SHORT (HALTEN) | 68%→52% | 50% schließen empfohlen |
| 13.02 | IREN | HOLD | 55% | Verkauft 13.02 |
| 13.02 | SHOP | HOLD/WARTEN | 45% | Nicht gekauft |
| 13.02 | HOOD | LONG (BOUNCE) | 58% | Gekauft, Stop ausgelöst -57 EUR |
| 13.02 | VST | HOLD | 62% | 50% sichern bis 24.02! |
| 17.02 | VST | HOLD+TEIL | 70% | 50 St. verkauft! |
| 13.02 | APLD | WARTEN | 48% | Nicht gekauft |
| 17.02 | HOOD | SELL | 72%→65% | Stop ausgelöst |
| 17.02 | ASML | HOLD/WARTEN | 55% | Nicht gekauft |
| 17.02 | CEG | LONG (nach Earnings) | 62% | Nicht gekauft (kein Cash) |
| 17.02 | ENR.DE | LONG (SOFORT!) | 75% | GEKAUFT 18.02! Stop bei ~Break-Even ausgelöst 26.02 |
| 22.02 | ENR.DE | HOLD (Re-Analyse) | 62% | Gestaffelte Exits 168/185/200 |
| 17.02 | ARM | HOLD | 52% | Verkauft 18.02 |
| 18.02 | SI=F | WARTEN | 55% | Später LONG gekauft! +10% Stop ausgelöst |
| 19.02 | NBIS | LONG | 72% | Nicht gekauft (kein Cash?) |
| 18.02 | RKLB | LONG (taktisch) | 63% | Gekauft, dann verkauft |
| 18.02 | SAP.DE | LONG | 62% | Nicht gekauft |
| 18.02 | APLD | WARTEN | 52% | Nicht gekauft |
| 23.02 | NVO | SHORT | 68% | GEKAUFT + VERKAUFT bei +20%! Kern-Strategie! |
| 23.02 | VST | HOLD + 50% SICHERN | 70% | Stop bei +50% ausgelöst 26.02 |
| 23.02 | BTC-USD | SHORT (informativ) | 72% | Nicht gehandelt (kein Cash/Slots) |
| 26.02 | ENR.DE | Chart-Update | - | ATH 169.40! RSI 72.9 overbought, bearische Divergenz |
| 26.02 | VST | Chart-Update | - | $175.36, RSI 83.9! Earnings HEUTE |
| 26.02 | AMZN | LONG (TAKTISCH) | 62% | Entry $205-208, KO $194, Stop $199, Exits $220/$227/$235 |
| 26.02 | TSM | SHORT (KORREKTUR) | 65% | Entry $375-380, KO $404, Stop $392, Cover $360/$345/$330 |
| 26.02 | VST | HOLD/WARTEN | 48% | RSI 84 extrem, Death Cross, Trigger: LONG >$181, SHORT <$165 |

</details>
