...
23. **Aktive Cert-Aufforderung auch bei knapp-NO-TRADE** — Bei Signal = LONG/SHORT immer Cert-Aufforderung an User anhängen (Typ, KO, Hebel nach ATR-Tabelle, Ask-Preis, Trade Republic). Bei NO-TRADE mit Confidence 55-59% ("knapp verfehlt") ebenfalls eine **Stand-by-Cert-Aufforderung** mit konkretem Flip-Trigger ("falls morgen X eintritt"). Bei NO-TRADE <55% oder ohne qualitatives Setup: keine Cert-Aufforderung, stattdessen "Kein Setup in Reichweite". Hebel-Mapping aus ATR%: <3% → 6-10×, 3-5% → 4-6×, 5-7% → 3-4×, >7% → Warrants/Options (V1-Veto). **Why:** User soll nicht raten, ob er ein Cert suchen soll. Bei 58% Confidence wie HOOD ist das Setup real — nur eine Metrik kippt es. Stand-by-Cert spart den zweiten Suchlauf, wenn Re-Run morgen Gate öffnet. **How to apply:** Step 4 hat explizite Template-Blöcke für die drei Fälle (LONG/SHORT, knapp-NO-TRADE, klar-NO-TRADE).
